...
#orig_height = int(input("crop height: ")) # 147
#orig_width = int(input("crop width: ")) # 147
orig_height = 147
orig_width = 147
...
while index < len(lines):
    ...
    t2w_file = nib.load('../data/mri/{}/t2w.nii'.format(patient_id))
    t2w_image = t2w_file.get_fdata()
    t2w_image_sliced = t2w_image[:,:,k]

    # Saving the sliced image
    save_path = '../data/mri/{}/t2w_sliced_0{}.nii'.format(patient_id, fid)
    ...
    save_path = '../data/mri/{}/t2w_cropped_0{}.nii'.format(patient_id, fid)
    skimage.io.imsave(save_path, t2w_image_cropped)
    
    # Saving cropped image as png
    save_path = '../data/test/crop_t2w/{}_{}.png'.format(patient_id, fid)
    skimage.io.imsave(save_path, t2w_image_cropped)
